...
Aqui, $\textrm{logito}(p_{it}) = log(p_{it}/(1-p_{it})$; $j$ indexa a variável explicativa escalar e $k$, a funcional; $J$ e $K$ são o número de covariáveis escalares e covariáveis funcionais no modelo, respectivamente. $\beta_j$ é efeito das covariável escalar $j$ no logito; $f_k(x)$ são funções suaves de $x_{ik(t-1,t-2,...,t-p)}$; $t-1$, $t-2$ e $t-p$ indexam 1, 2 e $p$ dias anteriores ao dia $t$, respectivamente. A função $f_k(x)$ será estimada através de 

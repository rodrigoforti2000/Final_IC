...
$$ \textrm{logito}(p_{it}) = \beta_0 + \sum_{j=1}^{J}\beta_jx_{ij(t-1,t-2,...,t-r)} +  \sum_{k=1}^{K}f_k(x_{ik(t-1,t-2,...,t-r)})
+ u_{i} \tag{2}.$$ 
...
$$\hat{f_k}(x) = \sum_{s=1}^{S}B_s(x)\hat{\theta}_s \tag{3},$$ onde $B_s$ são as funções bases e $\hat{\theta}_s$ seus respectivos coeficientes.
...
Na sequência, realizamos 10 repartições, sendo que, para cada repartição, os envelopes foram aleatorizados em 9 envelopes treino e um envelope teste. Em cada repartição, os modelos foram treinados com as observações de 9 dos 10 envelopes; e o envelope teste foi utilizado para teste do modelo. A Figura 1 traz um diagrama ilustrativo da validação cruzada que foi empregada. Para mais informações sobre validação cruzada, veja Kohavi (2007).
...
tab_esc_fun = as.data.frame(mol1_gam$p.table) %>% tibble::rownames_to_column("term") %>%
        ...
        mutate(term=replace(term, term == "(Intercept)", "Intercepto")) %>%
        mutate(term=replace(term, term=="doente_k11", "doente_t1")) %>%
        mutate(term=replace(term, term=="doente_k21", "doente_t2")) %>%
        mutate(term=replace(term, term == "doente_k11:doente_k21", "doente_t1:doente_t2")) %>%
        mutate(term=replace(term, term == "RuminacaoUltimas24h_k1", "RuminacaoUltimas24h_t1")) %>%
        mutate(term=replace(term, term == "RuminacaoUltimas24h_k2", "RuminacaoUltimas24h_t2")) %>%
        mutate(term=replace(term, term == "AlimentacaoUltimas24h_k1", "AlimentacaoUltimas24h_t1")) %>%
        mutate(term=replace(term, term == "AlimentacaoUltimas24h_k2", "AlimentacaoUltimas24h_t2")) %>%
        mutate(term=replace(term, term == "RuminacaoUltimas24h_k1:RuminacaoUltimas24h_k2", "RuminacaoUltimas24h_t1:RuminacaoUltimas24h_t2")) %>%
        mutate(term=replace(term, term == "AlimentacaoUltimas24h_k1:AlimentacaoUltimas24h_k2", "AlimentacaoUltimas24h_t1:AlimentacaoUltimas24h_t2")) %>%
        mutate(term=replace(term, term == "SICK_ALL1", "doente_t2")) %>%
...
kable(tab_esc_fun,
        caption = "Estimativas dos parâmetros do ajuste do modelo misto escalar-funcional. Os sufixos t1 e t2 são referentes à t-1 e t-2, respectivamente.",
        col.names = c("Termo","Estimativa","Erro padrão","Estatística do teste","P-valor"))  %>%
  kable_styling(font_size = 10, latex_options = "HOLD_position")
